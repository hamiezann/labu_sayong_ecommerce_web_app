...
$page = 'admin';
$subPage = 'manage-staff';
$pageName = 'Manage Staff';

// fetch full staff list
$staff_lists = mysqli_query($conn, "SELECT * FROM users WHERE role = 'staff' ORDER BY  acc_status DESC, FullName DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_staff'])) {
    ...
    // $password = mysqli_real_escape_string($conn, $defaultpassword);
    ...
    if (mysqli_query($conn, $insert)) {
        ...
        mysqli_query($conn, "INSERT INTO logs (user_id, action) VALUES ('$uid', 'Added Staff: $fullname')");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_staff'])) {
    ...
    if (mysqli_query($conn, $updateQuery)) {
        $uid = $_SESSION['user_id'];
        mysqli_query($conn, "INSERT INTO logs (user_id, action) VALUES ('$uid', 'Updated staff: $fullname')");
...
if (isset($_GET['delete'])) {
    ...
    // Step 1: Get staff info
    $result = mysqli_query($conn, "SELECT FullName, Image FROM users WHERE id = '$id' AND Role = 'staff' LIMIT 1");
    if (!$result || mysqli_num_rows($result) === 0) {
        $_SESSION['error_message'] = "❌ Staff not found or already deleted.";
        header("Location: manage-staff.php");
        exit();
    }
    ...
    // // mysqli_query($conn, "DELETE FROM products WHERE created_by = '$id'");
    // // mysqli_query($conn, "DELETE FROM orders WHERE staff_id = '$id'");
    ...
    $action = mysqli_real_escape_string($conn, "Deactivate account status for staff: $name");
    mysqli_query($conn, "INSERT INTO logs (user_id, action) VALUES ('$uid', '$action')");

    $_SESSION['success_message'] = "🗑️ Staff account '$name' deactivated successfully with related data!";
    header("Location: manage-staff.php");
    exit();
}
...
if (isset($_GET['toggle'])) {
    ...
    $query = mysqli_query($conn, "SELECT FullName, acc_status FROM users WHERE id='$id' AND role='staff'");
    if ($row = mysqli_fetch_assoc($query)) {
        $newStatus = $row['acc_status'] ? 0 : 1;
        $statusText = $newStatus ? 'Activated' : 'Deactivated';
        mysqli_query($conn, "UPDATE users SET acc_status='$newStatus' WHERE id='$id'");
        $uid = $_SESSION['user_id'] ?? 0;
        $action = mysqli_real_escape_string($conn, "$statusText staff: {$row['FullName']}");
        mysqli_query($conn, "INSERT INTO logs (user_id, action) VALUES ('$uid', '$action')");
        $_SESSION['success_message'] = "✅ Staff '{$row['FullName']}' has been $statusText.";
    }
    header("Location: manage-staff.php");
    exit();
}
    ...
    <div class="app-content p-4">
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-item-center mb-4">
                ...
                <button class="btn btn-md btn-success addBtn"><i class="bi bi-plus-lg me-2"></i> Add Staff</button>
            </div>
            ...
            <div class="card-shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0"><i class="bi bi-list-ul me-2"></i>Staff List</h5>
                </div>
                <div class="card-body">
                    <table id="staffTable" class="table table-striped align-middle">
                        ...
                        <tbody>
                            <?php $i = 1;
                            while ($row = mysqli_fetch_assoc($staff_lists)) : ?>
...
<div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form method="POST" enctype="multipart/form-data" class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title">Add New Staff Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
                <button type="submit" name="create_staff" class="btn btn-primary">Create</button>
            </div>
        </form>
    </div>
</div>
